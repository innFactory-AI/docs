...
description: Erstelle und bearbeite PowerPoint-Präsentationen mit companyFILES MCP-Tools.
---

Verwende diesen Skill, wenn du eine Präsentation erstellen, Folien generieren, einen Foliensatz aufbauen oder Inhalte als PowerPoint exportieren möchtest.
...
Append new slides to an existing presentation in-place. Identify the target with `document_id` **or** `librechat_file_id` (a LibreChat attachment, updated in-place on the shared volume). Accepts markdown or a typed slides array.
...
Replace a single slide's content. Useful for correcting a specific slide without regenerating the whole deck. Identify the target with `document_id` **or** `librechat_file_id`.
...
Inspect an existing presentation before editing. Accepts `document_id` **or** `librechat_file_id`:
...
Returns `{ document_id, librechat_file_id, filename, slide_count, slides: [{ slideNumber, title, textLines }] }`.
